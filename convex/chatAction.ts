...
async function callGemini(
  ...
  if (!apiKey) {
    throw new Error("Missing GEMINI_API_KEY for Convex chat action");
  }
...
async function callModelJson(
  ...
  try {
    ...
    console.warn("Gemini JSON call failed, falling back to Groq", error);
    return await callGroqJson(messages);
  }
}
...
async function callModelText(
  ...
  try {
    ...
    console.warn("Gemini text call failed, falling back to Groq", error);
...
async function extractOperationalOutcome(input: {
  ...
  const content = await callModelJson([
    {
      role: "system",
      content:
        "You detect operational habit commands and clarification follow-ups. " +
        "Return valid JSON only with keys intent, habitName, targetDate, targetTime, continuePendingAction, supersedePendingAction, clarificationQuestion. " +
        "intent must be one of reschedule_habit_time, skip_habit_for_date, ask_today_plan, ask_tomorrow_plan, or none. " +
        ...
        "Use only habit names that match the provided activeHabits. " +
  ...
  return normalizeOperationalExtraction(parseJsonObject(content));
}
...
function buildOperationalRoute(input: {
  ...
  return {
    route: {
      ...
      targetTime,
      resolvedHabit:
        intent === "ask_today_plan" || intent === "ask_tomorrow_plan"
          ? null
          : resolvedHabit,
...
function buildPlannerReply(input: {
  ...
  if (input.plan.items.length === 0) {
    return `${title} ga ada habit aktif yang perlu lo kerjain. Jangan buang harinya.`;
  }

  const lines = input.plan.items.map((item) => {
    if (item.skipped) {
      return `- ${item.habitName} ${item.scheduledTime}: skip sengaja${
        item.skipReason ? ` (${item.skipReason})` : ""
      }.`;
    }

    if (item.checkInStatus === "completed") {
      return `- ${item.habitName} ${item.scheduledTime}: sudah kelar.`;
    }

    if (item.checkInStatus === "missed") {
      return `- ${item.habitName} ${item.scheduledTime}: sudah miss.`;
    }

    if (item.checkInStatus === "bonus") {
      return `- ${item.habitName} ${item.scheduledTime}: bonus sudah masuk.`;
    }

    return `- ${item.habitName} ${item.scheduledTime}: ${item.riskNote}.`;
  });
  ...
  const content = await callModelText([
    {
      ...
      content:
        ...
        "For planner replies, use a short title and one flat line per habit. " +
...
export const sendMessage = action({
  ...
  handler: async (
    ...
    const [rawExtraction, operationalExtraction] = await Promise.all([
      extractChatOutcome({
        content,
        source: args.source,
        context,
