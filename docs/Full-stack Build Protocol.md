# 🚀 PRODUCTION FULL-STACK BUILD PROTOCOL

## ⚡ PRE-FLIGHT CHECKLIST (USER: FILL THIS BEFORE RUNNING)

# 🔥 **STREAK APP - COMPLETE USER FLOW (FINAL VERSION)**

_Hybrid approach: Dashboard untuk quick actions + Chat untuk conversations + AI proaktif sepanjang hari_
...
const extractedData = await extractWorkoutData("squats 4x8, leg press 3x12");
// Returns: {
//   exercises: [
...
- Redirect to Clerk-managed upgrade flow (or in-app dev upgrade action)
- After successful upgrade → immediately unlock unlimited messages
...
```css
--bg-primary: #000000 (pure black) --text-primary: #ffffff (pure white)
  --text-secondary: #999999 (gray) --accent-streak: #ff0000 (red for streaks)
  --accent-success: #00ff00 (green, minimal use) --accent-missed: #666666
  (dark gray) --border: #333333 (subtle borders);
```
...
```css
--font-heading:
  "JetBrains Mono", monospace --font-body: "Inter",
  sans-serif --font-ai: "Courier New", monospace h1: 24px, 700 weight h2: 18px,
  700 weight body: 16px, 400 weight small: 14px, 400 weight;
```

### Layout & Responsiveness

- Support web desktop and web mobile
...
### **AI Functions (Convex Actions with Groq API)**
...
1. USER PROFILE
  ...
  - Tracks: Clerk-backed subscription tier (Free/Pro) via user metadata, AI personality, onboarding status, daily message count
...
- Natural language workout extraction (Groq API in Convex actions)
...
Before starting, execute these commands and report results:

```bash
node --version  # Must be 20.x or higher
npm --version   # Must be 10.x or higher
pwd             # Confirm we're in /home/claude
```

**IF ANY VERSION IS INCOMPATIBLE:** STOP immediately and report the issue.

---

## 📦 STRICT TECH STACK & VERSIONS

Use EXACTLY these tools. Do not deviate without explicit user permission:

| Technology          | Version/Config                       | Installation Method                                                |
| ------------------- | ------------------------------------ | ------------------------------------------------------------------ |
| **Next.js**         | 15.x (App Router, TypeScript strict) | `npx create-next-app@latest`                                       |
| **Styling**         | Tailwind CSS 3.x + Shadcn UI         | Next.js CLI includes Tailwind; Shadcn via `npx shadcn@latest init` |
| **Authentication**  | Clerk (latest)                       | `npm install @clerk/nextjs`                                        |
| **Database**        | Convex (latest)                      | `npm install convex`                                               |
| **Billing (Dev)**   | Clerk-based tier management          | Included with `@clerk/nextjs`                                      |
| **Icons**           | lucide-react                         | Auto-installed with Shadcn                                         |
| **Package Manager** | npm (consistent throughout)          | N/A                                                                |

---

## 🧠 EXECUTION PHILOSOPHY (CRITICAL RULES)

### ✅ DO:

- **Use CLIs for scaffolding** - Never manually write `package.json`, `next.config.js`, `tailwind.config.ts` from scratch
- **Write complete code** - No `// TODO`, `// Add logic here`, or placeholder comments
- **Use Shadcn for ALL UI components** - Run `npx shadcn@latest add <component>` before building custom UI
- **Stop at checkpoints** - Wait for "Proceed to Phase X" before continuing
- **Show command outputs** - Always display the result of CLI commands

### ❌ DON'T:

- Never assume a Shadcn component name - use the STOP & ASK protocol below
- Never install packages from memory - verify exact package names first
- Never skip error messages - always show full error output
- Never build multiple phases in one response - respect the step boundaries
...
1. **STOP IMMEDIATELY** - Do not attempt auto-recovery
2. **Show the FULL error output** (not summarized)
3. **Ask for guidance:**
   ...
   A) [Suggested fix 1]
   B) [Suggested fix 2]
   C) Wait for your manual intervention?
   ```

---

## 🎯 COMPONENT DISCOVERY PROTOCOL (SHADCN)

**BEFORE building any UI component:**

1. Check if Shadcn has it: https://ui.shadcn.com/docs/components
2. **IF UNSURE** - Use the STOP & ASK rule:

   ```
   🛑 COMPONENT CHECK REQUIRED

   I need: [description - e.g., "a modal dialog with form validation"]

   Shadcn might have: [your best guess - e.g., "dialog" or "alert-dialog"]

   Please confirm:
   - Exact Shadcn component name, OR
   - "Build custom" if no match exists
   ```

3. **ONLY AFTER CONFIRMATION** - Run `npx shadcn@latest add <component>`
...
│   │   └── custom/            # Your business logic components
│   ├── lib/
...
│       └── [entity].ts        # CRUD functions per entity
...
- Confirm `tailwind.config.ts` was created
- Run `npm run dev` to verify the dev server starts (then stop it with Ctrl+C)

**Checkpoint:** Report file structure and wait for "Proceed to Phase 1b"

---

#### **Phase 1b: Initialize Shadcn UI**
...
- Style: Default
- Base color: Slate (or as specified in UI aesthetic)
- CSS variables: Yes

**After completion:**

- Confirm `src/components/ui/` folder exists
- Confirm `src/lib/utils.ts` exists

**Checkpoint:** Wait for "Proceed to Phase 2a"

---

### **PHASE 2: AUTHENTICATION (CLERK)**

#### **Phase 2a: Install & Configure Clerk**
...
3. **STOP & REQUEST:** "I need your Clerk API keys. Please provide:
   - `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`
   - `CLERK_SECRET_KEY`

   Add them to `.env.local` and confirm when ready."

**Checkpoint:** Wait for env vars confirmation, then "Proceed to Phase 2b"

---

#### **Phase 2b: Build Auth Pages**
...
Use Clerk's prebuilt `<SignIn />` and `<SignUp />` components wrapped in Shadcn cards.

**Create protected route:**

- `src/app/(protected)/dashboard/page.tsx`
- Use `auth()` from `@clerk/nextjs/server` to get user data
- Display: "Welcome, [user.firstName]!"

**Test:** Visit `/dashboard` without signing in → should redirect to sign-in

**Checkpoint:** Demo the auth flow, then wait for "Proceed to Phase 3a"

---

### **PHASE 3: DATABASE (CONVEX)**

#### **Phase 3a: Initialize Convex**
...
- Open browser for Convex account setup
- Create `convex/` folder
- Generate `convex.json`

**STOP & REQUEST:** "Please complete Convex setup in browser, then provide the `CONVEX_URL` and `NEXT_PUBLIC_CONVEX_URL` from the dashboard."

**After receiving env vars:**

- Add to `.env.local`
- Confirm `convex/_generated/` folder exists

**Checkpoint:** Wait for "Proceed to Phase 3b"

---

#### **Phase 3b: Design Schema & CRUD**

**Based on user's entity specifications, create `convex/schema.ts`:**

Example structure (adapt to user's entities):
...
export default defineSchema({
  ...
  // Entity 2 - adapt to user specs
  [entity2Name]: defineTable({
    userId: v.id("users"),
    // Add fields from user specification
  }).index("by_user", ["userId"]),

  // Entity 3 - adapt to user specs
  [entity3Name]: defineTable({
    [entity2Id]: v.id("[entity2Name]"),
    // Add fields from user specification
  }).index("by_[entity2]", ["[entity2Id]"]),
});
```

**Create CRUD files:**

- `convex/users.ts` - syncUser mutation (called from Clerk webhook)
- `convex/[entity2].ts` - create, read, update, delete mutations/queries
- `convex/[entity3].ts` - same pattern
...
**Checkpoint:** Show schema and wait for "Proceed to Phase 3c"

---

#### **Phase 3c: Clerk-Convex User Sync**

**Create webhook handler:**
`src/app/api/webhooks/clerk/route.ts`

Use `@clerk/nextjs/server` webhook verification + Convex mutation to sync user data.

**STOP & REQUEST:** "I need your Clerk webhook secret (`CLERK_WEBHOOK_SECRET`). Please:

1. Go to Clerk Dashboard → Webhooks
2. Create endpoint: `https://[your-domain]/api/webhooks/clerk`
3. Enable 'user.created' and 'user.updated' events
4. Provide the signing secret"

**Checkpoint:** Wait for secret, test webhook, then "Proceed to Phase 4a"

---

### **PHASE 4: FRONTEND INTEGRATION**

#### **Phase 4a: Setup Convex Provider**
...
**Wrap app with ConvexProviderWithClerk** in `src/app/layout.tsx`

**Checkpoint:** Confirm no errors, wait for "Proceed to Phase 4b"

---

#### **Phase 4b: Build Dashboard UI**

**Run component installations as needed:**

```bash
# Example - adjust based on UI needs
npx shadcn@latest add table badge dialog dropdown-menu
```

**Build:**

- Dashboard layout using Shadcn components
- Connect to Convex queries using `useQuery` hook
- Implement CRUD operations using `useMutation` hook

**IMPORTANT:** Build with REAL data, not mock data. The Convex backend is already live.

**Apply UI aesthetic** from user specification (dark mode, typography, spacing)

**Checkpoint:** Demo the working dashboard, wait for "Proceed to Phase 5a"

---

### **PHASE 5: BILLING (CLERK - SIMPLE DEV MODE)**

#### **Phase 5a: Implement Clerk-Based Tier Source of Truth**

No new package install needed (already using `@clerk/nextjs`).

**Implement:**

- Store `subscriptionTier` (`free` or `pro`) in Clerk user public metadata.
- On app load, sync Clerk metadata into Convex `users.subscriptionTier` so backend gating stays consistent.
- Keep all limits enforced from Convex (`3 habits free`, `20 messages/day free`).

**Create/Update:**

- `src/app/api/billing/upgrade/route.ts` to set tier to `pro` for the current user in development.
- `src/app/api/billing/downgrade/route.ts` to set tier back to `free` (optional but recommended).
- Upgrade UI action to call these routes instead of external checkout.

**Checkpoint:** Demo tier switching and limit unlock behavior, then wait for "Proceed to Phase 5b"

---

#### **Phase 5b: Future-Ready Migration Note (Polar Later)**

For production billing later, replace the dev upgrade routes with Polar checkout + webhook flow.

**Future migration targets:**

- Add checkout session creation endpoint
- Add webhook handler: `src/app/api/webhooks/polar/route.ts`
- Keep Convex tier update logic unchanged (only tier source changes)

**Checkpoint:** Confirm architecture is migration-ready, wait for "Proceed to Phase 6"

---

### **PHASE 6: POLISH & DEPLOYMENT**

#### **Phase 6a: Final Touches**

- Add loading states (Shadcn skeleton components)
- Add error boundaries
- Implement responsive design (mobile-first with Tailwind)
- Add metadata for SEO (`metadata` export in pages)

#### **Phase 6b: Pre-Deployment Checklist**
...
- [ ] UI matches specified aesthetic

#### **Phase 6c: Vercel Deployment**

**If user confirmed Vercel as target:**
...
- Connect Convex deployment URL
- Add all environment variables in Vercel dashboard
- Set up Clerk webhook URLs to production domain (and Polar webhook later when migrating billing)

**FINAL CHECKPOINT:** Provide:

1. Live production URL
2. Test credentials (if applicable)
3. Summary of all features implemented
4. Known limitations or future enhancements

---

## 📋 TESTING POLICY

**By default:** Focus on working features. Do NOT write Jest/Vitest tests unless explicitly requested.

**If user requests testing:**

- Install: `npm install -D vitest @testing-library/react`
- Create: `src/__tests__/` folder
- Write: Unit tests for Convex functions, integration tests for critical flows
...
# Clerk billing (dev mode)
NEXT_PUBLIC_BILLING_MODE=clerk_dev

# Future migration (optional, keep empty for now)
# POLAR_ACCESS_TOKEN=
# POLAR_ORGANIZATION_ID=
# POLAR_WEBHOOK_SECRET=
```

**Create `.env.example` (committed to git) with empty values as template**

---

## 🎬 INITIALIZATION COMMAND

**After user fills pre-flight checklist, respond with:**

```
✅ Pre-flight checklist received.
...
[Run and show: node --version, npm --version, pwd]

Ready to execute Phase 1a: Initialize Next.js?

Reply "Proceed to Phase 1a" to begin.
```
...
1. **NEVER skip checkpoints** - wait for explicit "Proceed to Phase X" commands
2. **NEVER guess Shadcn component names** - use STOP & ASK protocol
3. **NEVER auto-recover from errors** - use error handling protocol
4. **ALWAYS show command outputs** - don't just say "I ran X"
5. **RESPECT rate limits** - if too many operations, pause and ask to continue
